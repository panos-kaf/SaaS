...
class MessagingSubscriber {
  ...
  async processGradeMessage(grade) {
    try {
      ...
      // If submission doesn't exist, create it with available data
      if (submissionCheck.rows.length === 0) {
        // Try to derive course_id from the grade data
        const courseId = grade.course_id || `${grade.course_code || 'UNKNOWN'}-${grade.semester || 'UNKNOWN'}`;
        
        await db.query(
          `INSERT INTO grade_submissions (
            submission_id, course_id, prof_id, semester, submission_date, is_finalized
          ) VALUES ($1, $2, $3, $4, $5, $6)`,
          [
            grade.submission_id,
            courseId,
            grade.prof_id || null,
            grade.semester || 'Unknown',
            new Date(),
            false
          ]
        );
        console.log(`Created grade submission record: ${grade.submission_id} for course: ${courseId}`);
      }
      ...
      if (gradeCheck.rows.length === 0) {
        ...
        await db.query(
          `UPDATE grades SET 
            prof_id = $2, student_academic_number = $3,
            student_name = $4, student_email = $5, semester = $6,
            course_name = $7, course_code = $8, grade_scale = $9,
            grade = $10, submission_id = $11, updated_at = CURRENT_TIMESTAMP
          WHERE grades_id = $1`,
          [
            grade.grades_id, grade.prof_id, 
            grade.student_academic_number, grade.student_name, 
      ...
      // Update statistics - get course_id from the submission
      const submissionResult = await db.query(
        'SELECT course_id FROM grade_submissions WHERE submission_id = $1',
        [grade.submission_id]
      );
      
      if (submissionResult.rows.length > 0) {
        await this.updateStatistics(submissionResult.rows[0].course_id);
      }
    } catch (error) {
  ...
  async updateStatistics(courseId) {
    try {
      // Get all grades for the course by joining with submissions
